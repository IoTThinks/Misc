...
void sendLoRaMessage(const char* outgoing) {
  ...
  LoRa.endPacket();                     // finish packet and send it. Can not use async due to IQ Invert
...
void loRa1ReadTask(void* pvParameters) {
  ...
  log("[LoRa] Receive message: ", incoming, false);
  log(", RSSI: ", LoRa.packetRssi(), false);
  log(", SNR: ", LoRa.packetSnr(), true);
...
void onLoRa1ReceiveCallback(int packetSize) {
  ...
  xTaskCreatePinnedToCore(loRa1ReadTask, "loRa1ReadTask", 10240, (void*)&packetSize, 
    CRONJOB_READLORA_PRIORITY, NULL, 0);
}

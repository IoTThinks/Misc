...
// Optimize log with char*
void log(const char* prefix1, const char* dynamicContent1, bool isNewLine)
{
  log(prefix1, false);
  log(dynamicContent1, isNewLine);
}
...
// Int version
void log(const char* prefix1, int dynamicContent1, bool isNewLine)
{
  char contentInCharPtr[16];
  itoa(dynamicContent1, contentInCharPtr, 10);
  log(prefix1, contentInCharPtr, isNewLine);
}

// Float version
void log(const char* prefix1, float dynamicContent1, bool isNewLine)
{
  char contentInCharPtr[16];
  sprintf(contentInCharPtr, "%.2f", dynamicContent1);  
  log(prefix1, contentInCharPtr, isNewLine);
}
